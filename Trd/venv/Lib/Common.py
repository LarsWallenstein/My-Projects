...
def load_data():  # 1. data, 2.prices
    time_frame = str(3)#str(random.randint(2,3))
    ...
    while True:
        ...
        d1 = np.array(pd.read_csv(PATH + '/' + "tvix.us.csv", sep=',').round(5))
    ...
    return d1,pd.read_csv(PATH_2 + '/' + csv_to_txt("tvix.us.csv"), sep=',').round(5)
