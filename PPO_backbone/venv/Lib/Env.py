...
class State:
    ...
    def shape(self):
        ...
        return (1, 49 * self.bars_count + 1 + 1 + 1)
    ...
    def encode(self):
        ...
        if not (self.long_position or self.short_position):
            ...
            res[0, -1] = (-1.0)*(self.open_price - self._cur_close()) / self.open_price
        return res
    ...
    def step(self, action):
        ...
        if self.long_position and action == Actions.Sell:
        ...
        elif self.long_position and action == Actions.S_Sell:
            self.long_position = False
        ...
        elif self.short_position and action == Actions.B_Buy:
            self.short_position = False
            ...
            if action == Actions.Buy or action == Actions.B_Buy:
                self.long_position = True
            else:
                self.short_position = True
            self.open_price = close
        ...
        mean_3 = 0.0
        mean_6 = 0.0
        for i in range(1,min(7,self._prices.shape[0]-self._offset)):
            if i<4:
                mean_3+=(self._prices.loc[self._offset+i, 'Close']+self._prices.loc[self._offset+i, 'Open'])/2.0
                mean_6 += (self._prices.loc[self._offset + i, 'Close'] + self._prices.loc[self._offset + i, 'Open']) / 2.0
            else:
                mean_6 += (self._prices.loc[self._offset + i, 'Close'] + self._prices.loc[self._offset + i, 'Open']) / 2.0
        mean_3 = (1.0*mean_3)/3
        mean_6 = (1.0*mean_6)/6
        info = (mean_3-self._cur_close(), mean_6-self._cur_close())
...
class StocksEnv(gym.Env):
    ...
    def step(self, action_idx):
        if(len(action_idx)>0):
            idx = np.array(action_idx).argmax(axis=0)
            action_idx =idx
        action = Actions(action_idx)
        reward, done, info = self._state.step(action)
